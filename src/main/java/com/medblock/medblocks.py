...
def decode_creds(string):
    decode_string = base64.b64decode(string).decode('utf-8').replace('\n', '\\n')[:-4]
    user = load_user(decode_string)
    return user
...
class Patient(object):
    ...
    def has_permission(self, medblock):
        current_user = decode_creds(creds)
    ...
    def write_medblock(self, ipfs_hash, encrypted_key, creds, emergency_key=None, format=None):
        current_user = decode_creds(creds)
        data = {
        ...
        self.transfer_medblock(sent, creds)

    def transfer_medblock(self, tx, creds, metadata=None):
        current_user = decode_creds(creds)
...
def ipfs_connect():
    try:
        api = ipfsapi.connect('ipfs_host', 5001)
...
@click.option('--utype','-ut')
def createuser(name, phone, email, utype, output):
    ...
    with open(output,'w') as f:
        f.write(st)
    print(st)
    reg_user = load(output)
    register_on_blockchain(dump_user(reg_user))

@main.command()
def register():
    """Register current user on the blockchain"""
    user = decode_creds(creds)
...
def add(file, phone, address, creds, emergency):
    ...
    if emergency:
        ...
        patient.write_medblock(ipfs_hash, encrypted_key, creds, format=format)
